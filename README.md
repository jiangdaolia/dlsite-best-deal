...
- 保留上游史低标签原样，并用同格式单独显示 `本次可到 88円  93OFF`；不高于史低用金黄色，高于史低用蓝灰色
- 点击“本次可到”可查看入选计算路径、购物车当前/理论最低价、门槛差、更省的自动拆单方案和稍后再买拼单推荐
- 价格弹框同时显示日元与按 DLsite 当页比例估算的人民币；详细计算说明默认折叠
- 详情、购物车和浏览列表会为不依赖其他作品即可达理论价的作品显示 `单买即最优`
...
- 所有浏览列表都可在 DLsite 当前顺序上额外选择“最高可达到折扣”或“理论低价优先”，并可按需要凑件的具体活动/优惠券筛选
- 两种助手排序均以“当前平台价不高于史低”作为主键相同时的第二判断，再保持 DLsite 原顺序
- 购物车“稍后再买”保留排序开关，只提供“最高可达到折扣”和“理论低价优先”
- 列表未显示声优时，从当页批量作品数据补全全部声优
...
优惠券会区分“从未读取”和“已读取但为空”。读取后不会定时轮询；只有已知优惠券到期、成功购买、切换账号，或主动打开优惠券页时才刷新。作品元数据缓存 30 分钟，每次批量最多读取 100 个尚未缓存的编号；只有无限滚动或 SPA 在同页加载出新作品时才读取后续批次，不预取其他分页。
...
购物车快照不存在时读取一次；之后只在成功加入购物车或购物车页面内容变化后更新，并分别保存“立即购买”和“稍后再买”。所有计算、拆单和拼单推荐均为只读，脚本不会自动加购、套券、移动作品或提交订单。
作品移入、移出、删除或移到稍后再买时，脚本会对两个集合的变化去重检测，仅在内容真正改变时重算。
