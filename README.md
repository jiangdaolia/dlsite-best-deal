...
Via 等支持 `.user.js` 的浏览器可以直接打开链接安装。安装后登录 DLsite：打开“我的优惠券”页面可查看读取诊断；打开购物车会在人民币等本地化价格后补充精确的日元现价，并在每部作品旁显示可用券。当前可用的券按优惠力度降序排列，未满足件数或金额门槛的券灰显并说明差额。“稍后再买”的作品会按“只把这一部移入立即购买后”的情形预览，不会被误算进当前订单金额或件数。底部浮动状态提示可点按隐藏。
